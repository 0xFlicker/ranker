...
function arrayEq<T>(a1: Array<T>, a2: Array<T>): boolean {
  return a1.length === a2.length && a1.every((a, i) => a === a2[i]);
}

function pRange<T>(array: T[], start?: number, stop?: number, step?: number) {
  return array.filter((_: unknown, i: number) => {
    return (
...
async function createRanker({
  ...
  async function getMultipleNodes(
    ...
    const nodes = nodeRequest.Responses?.nodes
      ? (nodeRequest.Responses?.nodes).reduce((memo, result) => {
  ...
  async function increment(
    ...
    for (const [[key, child], amount] of nodeChildrenEntries) {
      if (amount !== 0) {
    ...
    //   DynamoDBDocumentClient.TransactWriteItem,
    //   void
    // >(
    ...
    if (
      ...
      try {
        await db.send(new BatchWriteCommand(writeRequest));
      } catch (e) {
  ...
  > {
    ...
    const saveNodes: [string, TableScores][] = scoreDocs?.Responses?.scores
      ? ((scoreDocs?.Responses?.scores).map((result: any) => [
          result.Player_ID,
          <TableScores>result,
        ]) as [string, TableScores][])
      : ([] as [string, TableScores][]);
    const oldScores = {};
    for (const [key, oldScore] of saveNodes) {
      if (oldScore) {
    ...
    for (const { Player_ID: id, Score: value } of scoresToRemove) {
      scoreDeltas.set(value, (scoreDeltas.get(value) || 0) - 1);
      scoreEntsDel.push(id);
    }
    for (const { playerId: id, date, score: value } of scores) {
  ...
  function computeNodeModifications(scoreDeltas: ScoreDeltaMap): ScoreDeltaMap {
    ...
    for (const [score, delta] of scoreDeltas.entries()) {
      log(() => `Computing node score: ${score}, delta: ${delta}`);
      for (const [nodeId, child] of findNodeIds(score as number[])) {
  ...
  function findRankForNodes(
    ...
    let tot = 0;
    for (const [nodeId, child] of nodeIdsWithChildren) {
  ...
  async function findScoreRecursive(
    ...
    const node = await db.send(
      new GetCommand({
        TableName: getTableName("nodes"),
        Key: {
  ...
  async function expiredScores(): Promise<TableScores[]> {
    ...
    const rootResponse = await db.send(
      new GetCommand({
        TableName: getTableName("leaderboards"),
        Key: {
          Board_Name: rootKey,
        },
      })
  ...
  async function totalRankedScore(): Promise<number> {
    const rootResult = await db.send(
      new GetCommand({
        TableName: getTableName("nodes"),
        Key: {
  ...
  async function leaderboardUpdate(
    ...
    const leaderboardResponse = await db.send(
      new GetCommand({
        TableName: getTableName("leaderboards"),
        Key: {
          Board_Name: rootKey,
